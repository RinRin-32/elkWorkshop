...
func (h *Handler) CreatePost(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&post); err != nil {
		h.Logger.Err(err).Msg("could not parse request from body")
		c.JSONP(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid request body: %s", err.Error())})
		return
	}
...
func (h *Handler) UpdatePost(c *gin.Context) {
	...
	if err = c.ShouldBindJSON(&post); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("cound not parse request : %s", err.Error())})
		return
	}
